...
import {getProjectsUrl, type Project} from "~/types/project";
...
const {data: projects} = await useFetch<Project[]>(getProjectsUrl(), {
  headers: {
        ...
        <UCard>
          <template #header>
            <div class="flex w-full">
              <UAvatar
                  size="xl"
                  :src=project.collaborators[0].user.profile.avatar
                  :alt=project.collaborators[0].user.username
              />
              <div class="flex-1 ml-4 truncate">
                <p class="truncate text-xl">{{ project.category }}</p>
                <p class="truncate text-sm">{{ project.description }}</p>
                <p class="truncate text-sm">{{ project.collaborators[0].user.username }}</p>
                <UBadge class="mt-2" color="white" :label="project.category"/>
              </div>
            </div>
          </template>
          <template #footer>
